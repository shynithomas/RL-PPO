...
class ConcurrentBayesianLearning():
    ...
    def choose(self, state, nodes_last_list, visits, decision_flag, nodes_last_goal=None):
        nodes = [None for i in range(CONST.NUM_AGENTS)]
        ...
        for agent in range(CONST.NUM_AGENTS):
            ...
            if not decision_flag[0][agent]:
                continue

            node_last_visit = []
            node_last_visit.append(
                obsMap.get_key(nodes_last_list[agent], obsMap.nodes))
            a = node_last_visit[0]
            ...
            if selected == -1:
                selected = nb_list[0]
                i = obsMap.patrol_nodes.index(selected)
                visitflag[i] = True
            else:
                i = obsMap.patrol_nodes.index(selected)
                visitflag[i] = True
            ...
            nodes[agent] = obsMap.nodes[selected]
